...
1. **Matthias Eduard Geisriegler. *Process Mining of Business Process Choreographies*[D], FH JOANNEUM - University of Applied Sciences, Graz, Austria, 2017.**  
摘要：以往的流程挖掘产生的模型是一个编制模型，而这篇硕士论文主要的贡献是、可以通过流程挖掘产生S-BPM建模方式的流程编排模型，并对进行了系统集成，S-BPM系统执行平台的体系结构如下图所示：<div align=center><img width="500" height="300" src="resources/architecture_of_S-BPM_execution_platform.jpg"/></div>
**主要思路**：
   - （1）扩充事件日志属性信息，使其包含通信的信息如下：<div align=center><img width="500" height="100" src="resources/event_log_of_S-BPM.jpg"/></div>
   - （2）使用开源工具ProM挖掘流程，对挖掘的Petri net编制流程进行编辑，添加通信接口，将WF net描述的流程转（PNML格式）为WFN或Workflow Module（EPNML格式）一个例子如下：<div align=center><img width="500" height="300" src="resources/compatible_worflow_modules.jpg"/></div>
